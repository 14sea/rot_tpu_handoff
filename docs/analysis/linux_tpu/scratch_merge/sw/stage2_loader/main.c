...
int main(void)
{
    ...
    /* Wait for mode selection. EPCS-persist build (BOOT_MODE_SELECT=2):
     * stage2 is baked into IMEM-ROM and runs at cold-boot with no host, so a
     * NO-BYTE timeout now defaults to SD-boot (autonomous Linux from the
     * NEOLNX blob on the SD card) instead of U-Boot. A host can still send a
     * mode byte within the 3 s window to drive dev flows (l/b/W/R/...), and
     * U-Boot stays reachable via an explicit 'u'. */
    uart_puts("[stage2] l=xmodem s=smoke d=dump w=wtest W=wmulti R=rdhdr B=baud b/timeout=bootSD u=U-Boot\r\n");
     ...
     * R then W. Terminal modes (l, b, d, u) never return. */
    while (1) {
        if (mode == 'l') {
            mode_linux();              /* jumps to kernel, no return */
        } else if (mode == 'b' || mode < 0) {
            /* mode < 0 = 3 s timeout, no host: autonomous SD cold-boot. */
